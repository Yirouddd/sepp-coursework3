...
public class EventPerformanceController extends Controller {
    ...
    public Event createEvent() {
        ...
        if (!checkCurrentUserIsEntertainmentProvider()) {
            ...
            try {
                type = enums.EventType.valueOf(typeInput);
            ...
            while (addMore.equalsIgnoreCase("yes")) {
                ...
                String outdoorInput = view.getInput("Is venue outdoor? (yes/no): ");
                String smokingInput = view.getInput("Is smoking allowed? (yes/no): ");
                String performerNames = view.getInput("Enter performer names: ");

                int capacity = Integer.parseInt(capacityInput);
                boolean isOutdoor = outdoorInput.equalsIgnoreCase("yes");
                boolean isSmoking = smokingInput.equalsIgnoreCase("yes");
                double ticketPrice = 0;
                int numTickets = 0;

                if (isTicketed) {
                    String priceInput = view.getInput("Enter ticket price: ");
                    String numInput = view.getInput("Enter number of tickets: ");
                    ticketPrice = Double.parseDouble(priceInput);
                    numTickets = Integer.parseInt(numInput);
                }

                java.time.LocalDateTime startDateTime = java.time.LocalDateTime.parse(
                        startInput,
                        java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")
                );
                java.time.LocalDateTime endDateTime = java.time.LocalDateTime.parse(
                        endInput,
                        java.time.format.DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm")
                );
                ...
                Performance perf = new Performance(
                        ...
                        startDateTime,
                        endDateTime,
                        new ArrayList<>(java.util.Arrays.asList(performerNames.split(","))),
                        venueAddress,
                        capacity,
                        isOutdoor,
                        isSmoking,
